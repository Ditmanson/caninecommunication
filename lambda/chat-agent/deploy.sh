...
REGION="us-east-2"
TABLE_NAME="caninecommunication-chatbot-limits"
FUNCTION_NAME="caninecommunication-chat-agent"
ROLE_NAME="caninecommunication-chat-agent-role"
...
if ! aws dynamodb describe-table --table-name "$TABLE_NAME" --region "$REGION" >/dev/null 2>&1; then
  ...
  echo "Enabling TTL on the 'ttl' attribute..."
...
if ! aws iam get-role --role-name "$ROLE_NAME" >/dev/null 2>&1; then
  ...
  aws iam put-role-policy \
    --role-name "$ROLE_NAME" \
    --policy-name "chat-agent-permissions" \
    --policy-document "{
      \"Version\": \"2012-10-17\",
      \"Statement\": [
